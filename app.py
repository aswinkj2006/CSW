...
app = Flask(__name__)
app.secret_key = os.getenv('SENTIMENT_WATCH_SECRET', 'sentiment_watchdog_secret_key_2024')
...
{{
    ...
    def fallback_analysis(self, text):
        ...
        return {
            ...
            "confidence": 0.7,
            "score": score,
            "language": "english" if self.is_english(text) else "unknown",
    ...
    def is_english(self, text):
        ...
        return (english_chars / max(len(text), 1)) > 0.8
...
class SentimentWatchdog:
    ...
    def set_csv_file(self, csv_file):
        """Set new CSV file and load data"""
        self.csv_file = csv_file
        self.load_data()
    
    def validate_csv_format(self, df):
        """Validate that the CSV has the required columns"""
        required = ['ticket_id', 'message', 'date', 'time']
    ...
    def load_data(self):
        try:
            ...
            self.df['sentiment'] = self.df['analysis'].apply(lambda x: x.get('sentiment', 'neutral'))
            self.df['sentiment_score'] = self.df['analysis'].apply(lambda x: x.get('score', 0.0))
            self.df['confidence'] = self.df['analysis'].apply(lambda x: x.get('confidence', 0.0))
            self.df['language'] = self.df['analysis'].apply(lambda x: x.get('language', 'unknown'))
            self.df['translation'] = self.df['analysis'].apply(lambda x: x.get('translation', ''))
            self.df['emotion'] = self.df['analysis'].apply(lambda x: x.get('emotion', 'neutral'))
            self.df['urgency'] = self.df['analysis'].apply(lambda x: x.get('urgency', 'medium'))
            self.df['category'] = self.df['analysis'].apply(lambda x: x.get('category', 'general'))
...
def api_all_tickets():
    tickets = []
    if not watchdog.df.empty:
        for _, row in watchdog.df.iterrows():
            tickets.append({
                ...
                'translation': row['translation'] if row['translation'] != row['message'] else None,
                'emotion': row['emotion'],
